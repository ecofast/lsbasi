...
func (self *Lexer) id() *Token {
	ret := ""
	for self.currChar != 0 && (unicode.IsLetter(self.currChar) || unicode.IsDigit(self.currChar)) {
		ret += string(self.currChar)
		self.advance()
	}
...
func (self *Lexer) getNextToken() *Token {
	for self.currChar != 0 {
		...
		if self.currChar == ':' {
			self.advance()
			return newToken(cTokenTypeOfColonSign, ':')
		}

		if self.currChar == ',' {
			self.advance()
			return newToken(cTokenTypeOfCommaSign, ',')
		}
		...
		if self.currChar == '/' {
			self.advance()
			return newToken(cTokenTypeOfFloatDivSign, '/')
		}
